...
public class SpearUnitChecker extends SpearSwitch<Unit> {
  ...
  public Unit caseSpecificationCall(SpecificationCall sc) {
    ...
    if (!args.equals(inputs)) {
      error(
          "Provided units of type " + args + ", but spec " + sc.getSpec().getName() + " expected units " + inputs + ".",
          sc, SpearPackage.Literals.SPECIFICATION_CALL__ARGS);
      return error(sc);
    }
  ...
  public Unit casePatternCall(PatternCall pc) {
    ...
    if (!args.equals(inputs)) {
      if (pc.getPattern().getName() == null) {
        ...
        error("Provided units of type " + args + ", but pattern " + pc.getPattern().getName() + " expected units "
            + inputs + ".", pc, SpearPackage.Literals.PATTERN_CALL__ARGS);
        return error(pc);
      }
    }
